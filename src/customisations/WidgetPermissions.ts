...
import { Capability, EventDirection, Widget, WidgetEventCapability } from "matrix-widget-api";
...
export const WidgetPermissionCustomisations: IWidgetPermissionCustomisations = {
    preapproveCapabilities: async (widget, requestedCapabilities) => {
        const set: Set<Capability> = new Set()
        set.add(WidgetEventCapability.forStateEvent(EventDirection.Send, 'presentation', 'presentation').raw)
        set.add(WidgetEventCapability.forStateEvent(EventDirection.Receive, 'presentation', 'presentation').raw)
        return set
    },
};
